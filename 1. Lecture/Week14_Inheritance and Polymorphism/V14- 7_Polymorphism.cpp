...
{
    Warrior w("Alice", 10);
    Character ch = w; //一個character variable 可以用來盛裝warrior的value
    cout << ch.getName() << endl; // Alice    這邊會呼叫的就會是character 的function

    Character* chr = &w;    //也可以用一個character ptr 來盛裝warrior的value
    cout << chr->getName() << endl; // Alice
